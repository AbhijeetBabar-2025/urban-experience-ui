...
When the user provides a reference site or image, the most common failure mode is drifting toward **social media / community platform aesthetics** instead of the chosen urban/cultural language.
...
- Bold sans-serif type used for engagement rather than urban authority
- Section rhythm that scrolls like a story or reel rather than advancing a neighborhood or district
...
- Color blocking borrowed from a brand identity rather than derived from city palette
- Micro-animations that feel like UI feedback rather than urban motion
- A "content-first, shell-second" layout that flattens spatial depth in favor of legibility at scale

**The fix:** If the reference is a social platform, content aggregator, or community site — extract only the structural dimensions (rhythm, density, navigation posture) and rewrite them entirely through the chosen artist and city. Do not let the reference's surface aesthetic overwrite the urban brief.

If the reference site's aesthetic is fundamentally incompatible with the chosen urban/cultural language, say so explicitly and ask the user whether to keep the city/vibe or pivot to a different reference approach.
...
- Decorative layers that do not support the chosen urban/cultural language
- An expensive-looking hero followed by ordinary template sections
- Interior pages that fall back to generic templates while only the homepage carries the city concept
- A shared section shell reused so aggressively that different page roles lose their neighborhood or district identity
- A new demo that reuses the previous demo's wireframe with only different colors, copy, or decorative treatment

## Require Urban Structure

- Give each page a clear rhythm: spectacle, information, pause, and payoff.
- Define the site-wide urban/music grammar first.
- Then define each major page role as its own neighborhood or district.
...
- Let the city reference influence composition, not only colors.
- Add environmental layers such as grain, fog, frames, glows, scan lines, or separators when they fit the urban/cultural language.
- Make one visual idea dominate the page. Supporting sections should echo it rather than introduce unrelated ideas.
- Make each major page role behave like a new neighborhood or district in the same city, with its own composition logic.
...
- Use simple motion only when restraint is part of the chosen city's language.
...
- Every standout visual device must justify itself against the chosen city and vibe.
...
- Avoid pure black and pure white unless the chosen city genuinely calls for them.
...
1. If the logo were removed, would the page still feel like the chosen city or vibe?
...
7. Do the interior pages still express the city through structure and pacing, or only through palette and type?
8. Would this page still have a unique identity if every card border and color treatment were removed?
9. Has the shared system supported the page neighborhood or districts, or flattened them?
...
12. If the user provided a reference site, does the output still feel like the chosen city — or has it drifted toward the reference's own aesthetic (social platform, brand grid, feed layout)?
