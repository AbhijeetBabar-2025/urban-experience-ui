...
The repository is maintained entirely in **English**.
...
- City + vibe must remain the emotional source
...
```
start questionnaire → research → decisions → storyboard → compiled-spec → build
```
...
This repository is maintained entirely in English to ensure professional standardization.
